...
subplot(1, 1, 1)
    ...
    xlabel('k');
    ylabel('x');
    legend({'Èñòèííîå x', 'Àïðèîðíàÿ îöåíêà x', 'Àïðèîðí. îöåíêà ÷åðåç ñòàöèîíàðíûé ôèëüòð'}, 'Location', 'northeast');
figure;
    subplot(1, 1, 1)
    plot(1:N, x_minus_star-x_minus, 'b');
    legend('Íåâÿçêà äëÿ àïðèîðíîé îöåíêè è àïð îöåíêè ÷åðåç ñòàöèîíàðíûé ôèëüòð', 'Location', 'northeast');
figure;
    subplot(1, 1, 1)
    plot(1:N, x_bezpomeh, 'b', 1:N, x_minus_bezpomeh, 'r');
    legend({'Èñòèííîå x áåç ïîìåõ', 'Àïðèîðíàÿ îöåíêà x áåç ïîìåõ'}, 'Location', 'northeast');
figure;
subplot(1, 1, 1)
    ...
    xlabel('k');
    ylabel('x');
    legend({'Èñòèííîå x', 'Àïðèîðíàÿ îöåíêà x', 'Àïîñòåðèîðíàÿ îöåíêà'}, 'Location', 'northeast');
end
